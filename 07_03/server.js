...
app.post("/messages", async (req, res) => { //app.post is HTTP POST request, we can use postman or browser
  var msg = new Message(req.body);
  var savedMsg = await msg.save();
  console.log('saved');
  // await Message.deleteMany({name:{$exists:true}});
  var censored = await Message.findOne({ message: "badword" });

  if (censored) {//if it finds 'badword' then it will be removed    
